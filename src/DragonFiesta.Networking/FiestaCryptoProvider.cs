namespace DragonFiesta.Networking
{
	public static class FiestaCryptoProvider
	{
		public static void Crypt(ref byte[] pBuffer, int pOffset, int pLen, ref ushort pPosition)
        {
                for (int i = 0; i < pLen; ++i)
                {
                    pBuffer[pOffset + i] ^= XorTable[pPosition];
                    pPosition++;    // did i forgot this? LOL
                    if (pPosition == XorTable.Length) pPosition = 0;
                }
        }
        public static void Crypt(ref byte[] pBuffer, ref ushort pPosition)
        {
            Crypt(ref pBuffer, 0, pBuffer.Length, ref pPosition);
        }

		public static readonly byte[] XorTable = new byte[]
			{
				
			};
	}
}