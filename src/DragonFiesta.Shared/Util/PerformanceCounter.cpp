...
{
    cpu_count = DragonFiesta::SysInfo::GetCPUCount();
    last_update = DragonFiesta::SysInfo::GetTickCount();
    last_cpu_usage = DragonFiesta::SysInfo::GetCPUUsage();
}

float PerformanceCounter::GetCurrentRAMUsage()
{
    unsigned long long usage = DragonFiesta::SysInfo::GetRAMUsage();
...
{
    unsigned long long now = DragonFiesta::SysInfo::GetTickCount();
    unsigned long long now_cpu_usage = DragonFiesta::SysInfo::GetCPUUsage();
