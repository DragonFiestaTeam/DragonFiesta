...
public:
    ...
    {
        return (boost::format("%1%:%2%") % GetRemoteIpAddress()  %  GetRemotePort()).str();
    }
    void AsyncRead()
    {
        if (!IsOpen())
    ...
    void ReadData(std::size_t size)
    {
        ...
        {
            sLog->LogMSG(MSG_DEBUG,LTAG_NETWORK, "Socket::ReadData: %s errored with: %i (%s)", GetRemoteIpAddress().to_string().c_str(), error.value(),
                         error.message().c_str());
    ...
    void CloseSocket()
    {
        ...
        if (shutdownError)
            sLog->LogMSG(MSG_DEBUG,LTAG_NETWORK,"Socket::CloseSocket: %s errored when shutting down socket: %i (%s)", GetRemoteIpAddress().to_string().c_str(),
