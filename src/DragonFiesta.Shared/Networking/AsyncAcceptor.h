    ...
    void AsyncAcceptManaged(ManagerAcceptHandler mgrHandler)
    {
        _acceptor.async_accept(_socket, [this, mgrHandler](boost::system::error_code error)
        {
            if (!error)
            {
                ...
                {
                    sLog->LogMSG(MSG_INFO,LTAG_NETWORK,"Failed to initialize client's socket %s", err.what());
                }
            }
...
// this-> is required here to fix an segmentation fault in gcc 4.7.2 - reason is lambdas in a templated class
            ...
            {
                sLog->LogMSG(MSG_INFO,LTAG_NETWORK,"Failed to retrieve client's remote address %s", err.what());
            }
        }
